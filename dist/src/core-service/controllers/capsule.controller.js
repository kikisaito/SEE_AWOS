...
exports.deleteCapsule = exports.updateCapsule = exports.getCapsules = exports.createCapsule = exports.requestUpload = exports.getPresignedUrl = void 0;
...
const getPresignedUrl = async (req, res) => {
    ...
    try {
        ...
        const { uploadUrl, key } = await (0, s3_service_1.generateUploadUrl)(userId, String(fileName), String(fileType));
        res.json({
            uploadUrl,
            s3Key: key
        });
...
const requestUpload = async (req, res) => {
    ...
    try {
        ...
        const data = await (0, s3_service_1.generateUploadUrl)(userId, String(fileName), String(fileType));
        res.json(data);
    }
    catch (error) {
...
const createCapsule = async (req, res) => {
    ...
    try {
        ...
        const newCapsule = await prisma_1.default.capsule.create({
            data: {
                userId,
                title,
                contentType,
                ...
                targetEmotions: {
                    connect: (emotionIds === null || emotionIds === void 0 ? void 0 : emotionIds.map((id) => ({ emotionId: id }))) || []
                }
            }
        });
...
const updateCapsule = async (req, res) => {
    ...
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: "No autorizado" });
        const existingCapsule = await prisma_1.default.capsule.findFirst({
        ...
        if (existingCapsule.contentType === 'AUDIO' && existingCapsule.s3Key) {
            if (contentType === 'TEXT' || (s3Key && s3Key !== existingCapsule.s3Key)) {
                try {
                    await (0, s3_service_1.deleteFileFromS3)(existingCapsule.s3Key);
        ...
        if (emotionIds) {
            dataToUpdate.targetEmotions = {
                set: [],
                connect: emotionIds.map((emotionId) => ({ emotionId }))
            };
        }
        const updatedCapsule = await prisma_1.default.capsule.update({
        ...
        if (updatedCapsule.contentType === 'AUDIO' && updatedCapsule.s3Key) {
            audioUrl = await (0, s3_service_1.getDownloadUrl)(updatedCapsule.s3Key);
        }
        res.status(200).json({
...
exports.updateCapsule = updateCapsule;
const deleteCapsule = async (req, res) => {
    ...
    try {
        ...
        await prisma_1.default.capsule.delete({
            where: {
                capsuleId: String(id)
            }
        });
